...
inMemoryCache.prototype = {
    ...
    return new Promise(function (onResolve, onReject) {
      ...
      for (var i = 0; i < keys.length; i++) {
        if (helper.validateCacheKey(keys[i]) === false) {
          return onResolve(responseHelper.error('l_c_im_mg_2', 'Cache key validation failed'));
        }
      }
   ...
   *
   * @param {string} key - cache key
   * @param {integer} byValue - number that you want to increment by. Default: 1
   *
   * @return {Promise<Result>} - On success, data.value is true. On failure, error details returned.
   */
  increment: function (key, byValue) {
    ...
    return new Promise(function (onResolve, onReject) {
      ...
      var record = oThis._getRecord(key);
      // To support memcached implementation
      if (!record) {
   ...
   *
   * @param {string} key - cache key
   * @param {integer} byValue - number that you want to decrement by. Default: 1
   *
   * @return {Promise<Result>} - On success, data.value is true. On failure, error details returned.
   */
   ...
   *
   * @param {string} key - cache key
   * @param {integer} lifetime - number that you want to set as expiry in seconds
   *
   * @return {Promise<Result>} - On success, data.value is true. On failure, error details returned.
   */
...
Record.prototype = {
  ...
  , setExpires: function (lifetimeInSec) {
    ...
    if (lifetime <= 0) {
      // To support redis implementation
